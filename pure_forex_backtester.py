...
def run_empirical_backtest():
    ...
    print("[*] Applying Pure Forex Filter (Eradicating Commodities & Indices)...")
    valid_symbols = {
    ...
    # Filter columns to only include the 27 valid forex pairs
    ...
    print("[*] Warping Chronological Time into Thermodynamic States (Tau) using ONLY FX Volume...")
    df['network_kinetic_energy'] = df[vol_cols].sum(axis=1)
    df['cumulative_energy'] = df['network_kinetic_energy'].cumsum()
    
    # Use standard energy threshold
    ...
    print(f"[+] Extracted {len(thermo_df)} Proper Time states (Pure FX).")
    ...
    print("[*] Igniting PyTorch Simulation (PURE FX THERMODYNAMICS)...")
    ...
    for i in range(50, len(pressure_df)):
        ...
        for sym, pos in portfolio.items():
            ...
            pnl_contribution = pos['size'] * pct_return * sim_return_multiplier
            step_pnl += pnl_contribution
            
            if current_phi < 0.5:
                ejected_positions.append((sym, pnl_contribution))
    ...
    print("\n" + "="*50)
    print("EMPIRICAL 6-MONTH PURE FOREX BACKTEST (OOS)")
    print("="*50)
