"""
AION PRIME HISTORICAL BACKTEST (STREAMING VERSION)
"""
import pandas as pd
import numpy as np
import glob
...
def main():
    ...
    print(f"[*] Streaming {len(files)} parquet chunks...\n")
    
    # Init active symbols by reading just one chunk (assumes all chunks have same symbols)
    first_df = pd.read_parquet(files[0])
    active_symbols = first_df['symbol'].unique().tolist()
    ...
    for f_idx, f in enumerate(files):
        print(f"[*] Processing Chunk {f_idx+1}/{len(files)}: {f}")
        df = pd.read_parquet(f)
        df.sort_values(by='time_msc', inplace=True)
            ...
            if global_tick_count >= GLOBAL_THRESHOLD:
                ...
                for t_id in list(shadow_trades.keys()):
                    ...
                    if state_idx - entry_state >= MAX_HOLD_STATES:
                        ...
                        notional = risk_amount / (sl_dist / trade['entry_price']) if sl_dist > 0 else 0
        ...
        # Explicit garbage collection after chunk
        gc.collect()

    # Close open trades at the end of stream
    for t_id in list(shadow_trades.keys()):
        trade  = shadow_trades[t_id]
        tsym   = trade['symbol']
        ...
        notional = trade['risk_amount'] / (sl_dist / trade['entry_price']) if sl_dist > 0 else 0
        commission = notional * COMMISSION_RATE
    ...
    pd.DataFrame(closed_trades).to_csv("aion_prime_trades_audit.csv", index=False)
    print("\n[+] Exported raw trades to aion_prime_trades_audit.csv")

    print_results(closed_trades, shadow_balance)
